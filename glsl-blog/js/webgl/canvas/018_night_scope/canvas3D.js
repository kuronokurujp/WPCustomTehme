...
class ExtendsCanvas3D extends Canvas3D {
    constructor() {
        ...
        this.saturation = 1.5;
        this.screenColor = [0.0, 1.0, 0.0];
        this.screenColorString = '#00ff00';
        this.sinWave = 100.0;
    ...
    setup(webGLFrame, shaderFrames) {
        ...
        */
        ...
        // ヴィネットの彩度
        {
            ...
            // 値のオブジェクト名と初期値設定の引数名と一致しないとエラーになる
            PANE.addInput({'vig': this.saturation}, 'vig', {min: 1.0, max: 2.0})
            .on('change', (v) => {
                this.saturation= v;
            });
        }
        ...
        // 走査線の周期
        {
            ...
            PANE.addInput({'sinWave': this.sinWave}, 'sinWave', {min: 100.0, max: 350.0})
            .on('change', (v) => {
                this.sinWave = v;
            });
        }
        ...
        // 走査線の強さ
        {
            ...
            PANE.addInput({'waveStrength': this.waveStrength}, 'waveStrength', {min: 0.0, max: 1.0})
            .on('change', (v) => {
                this.waveStrength = v;
            });
        }
        // スクリーン色
        {
            ...
            // 値のオブジェクト名と初期値設定の引数名と一致しないとエラーになる
            PANE.addInput({'color': this.screenColorString}, 'color')
            .on('change', (v) => {
                this.screenColorString = v;

                this.screenColor[0] = v.comps_[0] / 255.0;
                this.screenColor[1] = v.comps_[1] / 255.0;
                this.screenColor[2] = v.comps_[2] / 255.0;
            });
        }
        ...
        // ポストエフェクト用のシェーダー作成とそれに関する情報生成
        {
            ...
            // 画面色
            shaderFrame.createUniformLocation('screenColor', 'uniform3fv')
            // ヴィネットの彩度
            shaderFrame.createUniformLocation('saturation', 'uniform1f')
            // 走査線の周期
            shaderFrame.createUniformLocation('sinWave', 'uniform1f')
            // 走査線の強さ
            shaderFrame.createUniformLocation('waveStrength', 'uniform1f')
